...
export class ScannerPage {
  ...
  constructor(api: ApiService, storage: StorageService, ui: UI, onProductLogged: () => void) {
    this.api = api;
  ...
  async load() {
    ...
    this.searchInput = document.getElementById('product-search-input') as HTMLInputElement;
    this.barcodeInput = document.getElementById('barcode-search-input') as HTMLInputElement;
   ...
   */
  createMacroColumn(label: string, grams: number, color: string) {
    let percent = 0;
    if (grams > 0) {
