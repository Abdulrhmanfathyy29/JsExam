...
export class MealsPage {
  ...
  categoriesGrid = document.getElementById('categories-cards-grid');
  cuisinesBar = document.getElementById('cuisines-pills-container');
  searchInput = document.getElementById('recipe-search-input') as HTMLInputElement;
  countText = document.getElementById('recipes-count-text');
  gridViewBtn = document.getElementById('grid-view-btn');
  listViewBtn = document.getElementById('list-view-btn');
  viewAllBtn = document.getElementById('view-all-categories-btn');

  constructor(api: ApiService, ui: UI, openMeal: (mealId: string) => void) {
    this.api = api;
    this.ui = ui;
  ...
  async loadMeals() {
    ...
    if (this.searchText) {
      ...
      let results: Meal[] = await this.api.searchMeals(this.searchText);
  ...
  createMealCard(meal: Meal) {
    ...
    const noImage =
      '<div class="w-full h-full bg-gray-100 flex items-center justify-center"><i class="fa-solid fa-utensils text-gray-300"></i></div>';
