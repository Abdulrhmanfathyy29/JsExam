...
export class ApiService {
    ...
    async getMealById(id) {
        ...
        const results = await this.searchMeals(name);
        if (results.length > 0) {
     ...
     */
    async analyzeRecipeNutrition(recipeName, ingredients) {
    ...
    async searchProducts(query) {
        try {
            const data = await this.request('/products/search?q=' + encodeURIComponent(query) + '&page=1&limit=' + CONFIG.PRODUCTS_PER_PAGE);
            return data.results || [];
        }
        catch (error) {
     ...
     */
    async getOpenFoodFactsProduct(barcode) {
        try {
     ...
     * does not send them.
     */
    async addIngredientsAndAllergens(product) {
    ...
    async getProductsByCategory(category) {
        try {
            const data = await this.request('/products/category/' + encodeURIComponent(category) + '?page=1&limit=' + CONFIG.PRODUCTS_PER_PAGE);
            return data.results || [];
        }
        catch (error) {
